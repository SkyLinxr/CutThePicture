...
- (void)hideToastActivity {
    ...
    if (!existingActivityView) {
        existingActivityView = (UIView *)[self viewWithTag:1111];
    }
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height) + CSToastVerticalPadding);
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height)) - CSToastVerticalPadding - 50);
}
